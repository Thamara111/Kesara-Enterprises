...
<main class="bg-gray-50 py-12 min-h-screen">
    <div class="max-w-8xl mx-auto px-6 md:px-12">
        ...
        <nav class="flex items-center gap-2 text-xs font-medium text-gray-400 mb-8 overflow-x-auto whitespace-nowrap">
            ...
            <i class="ti ti-chevron-right text-[10px]"></i>
            <span
                class="text-gray-900 font-bold tracking-tight uppercase"><?= htmlspecialchars($product['name']) ?></span>
        </nav>
        ...
        <div class="grid lg:grid-cols-2 gap-12 items-start">
            ...
            <div class="space-y-10">
                ...
                <div class="space-y-4">
                    <div
                        class="bg-white border border-gray-100 rounded-3xl flex items-center justify-center shadow-sm relative overflow-hidden group aspect-square lg:aspect-video">
                        <?php if (!empty($primary_image)): ?>
                            <img id="main-product-image" src="<?= htmlspecialchars($primary_image) ?>"
                                alt="<?= htmlspecialchars($product['name']) ?>" class="w-full h-full object-cover">
                        <?php else: ?>
                            <i
                                class="ti ti-shirt text-[120px] text-gray-100 group-hover:scale-110 transition-transform duration-700"></i>
                        <?php endif; ?>
                ...
                <div class="bg-white border border-gray-100 rounded-3xl p-8 shadow-sm">
                    <h2
                        class="text-sm font-bold text-gray-900 uppercase tracking-widest mb-6 border-b border-gray-50 pb-4">
                        Product Specifications</h2>
                    <div class="grid grid-cols-2 gap-y-4 text-sm">
                        <span class="text-gray-400 font-medium">SKU</span>
                        <span
                            class="text-gray-900 font-bold text-right md:text-left"><?= htmlspecialchars($product['sku']) ?></span>

                        <span class="text-gray-400 font-medium">Material</span>
                        <span
                            class="text-gray-900 font-bold text-right md:text-left"><?= htmlspecialchars($prod_specs['material']) ?></span>

                        <span class="text-gray-400 font-medium">GSM</span>
                        <span
                            class="text-gray-900 font-bold text-right md:text-left"><?= htmlspecialchars($prod_specs['gsm']) ?></span>

                        <span class="text-gray-400 font-medium">Waistband</span>
                        <span
                            class="text-gray-900 font-bold text-right md:text-left"><?= htmlspecialchars($prod_specs['waistband']) ?></span>

                        <span class="text-gray-400 font-medium">Packaging</span>
                        <span
                            class="text-gray-900 font-bold text-right md:text-left"><?= htmlspecialchars($prod_specs['packaging']) ?></span>

                        <span class="text-gray-400 font-medium">Lead Time</span>
                        <span
                            class="text-gray-900 font-bold text-right md:text-left"><?= htmlspecialchars($prod_specs['lead']) ?></span>
                    </div>
                </div>
            ...
            <div class="bg-white border border-gray-100 rounded-3xl p-8 md:p-10 shadow-sm sticky top-24">
                ...
                <p class="text-xs font-medium text-gray-400 tracking-wider mb-4">SKU:
                    <?= htmlspecialchars($product['sku']) ?> &nbsp;·&nbsp; <?= htmlspecialchars($category_name) ?></p>

                <?php if ($discount > 0): ?>
                    <div
                        class="mb-6 inline-block px-3 py-1 bg-red-50 border border-red-100 rounded-lg text-red-600 font-bold text-xs uppercase tracking-widest">
                        <?= $discount ?>% OFF
                    </div>
                <?php endif; ?>
                            ...
                            <div class="flex justify-between items-center p-4 text-sm <?= $is_active ? 'bg-brand-light/30 border-y border-brand/10 text-brand' : 'bg-white text-gray-500' ?>"
                                ...
                                <div class="flex items-center gap-3">
                                    <span class="<?= $is_active ? 'font-bold' : '' ?>">
                                        ...
                                        <?= $t['max_qty'] ? ' – ' . htmlspecialchars($t['max_qty']) : '+' ?> units
                                    </span>
                                    <?php if ($is_active): ?>
                                        <span
                                            class="px-2 py-0.5 bg-brand text-brand-light text-[9px] font-bold rounded-full">YOUR
                                            QTY</span>
                                    <?php endif; ?>
                                </div>
                ...
                <div class="space-y-6 mb-8">
                    <div class="space-y-4">
                        <div class="flex justify-between items-center">
                            <label
                                class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest">Colour</label>
                            <span class="text-xs font-bold text-gray-500">Selected: <span id="selected-color-name"
                                    class="text-brand font-extrabold"><?= htmlspecialchars($default_colour) ?></span></span>
                        </div>
                        <div class="flex flex-wrap gap-4">
                            <?php foreach ($colours as $idx => $c):
                                ...
                                <div class="relative inline-block">
                                    <button id="color-btn-<?= $idx ?>" type="button"
                                        onclick="selectColor(<?= $idx ?>, '<?= htmlspecialchars($c) ?>')"
                                    ...
                                    <span id="color-qty-badge-<?= htmlspecialchars($c) ?>" class="shadow-md hidden"
                            ...
                            <?php if (empty($colours)): ?>
                                <div class="relative inline-block">
                                    <button
                                        class="w-10 h-10 rounded-full bg-white border border-gray-200 ring-2 ring-brand ring-offset-2 ring-offset-2 transition-all shadow-sm"
                                        title="Standard"></button>
                                    <span id="color-qty-badge-Standard" class="shadow-md hidden"
                                        style="background-color: #ff6600; color: #ffffff; position: absolute; top: -8px; right: -8px; padding: 2px 6px; font-size: 8px; font-weight: 900; line-height: 1; border-radius: 9999px; border: 1.5px solid #ffffff; z-index: 10;"></span>
                                </div>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="space-y-4">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest">Select
                            Quantity by Size</label>
                        <div
                            class="divide-y divide-gray-100 border border-gray-100 rounded-3xl overflow-hidden bg-white shadow-sm">
                            <?php
                ...
                <div class="bg-gray-50 rounded-2xl p-6 space-y-4 mb-8">
                    <div class="flex justify-between text-xs font-bold text-gray-400 uppercase tracking-widest">
                        ...
                        <span id="unit-price" class="text-gray-900">
                            <?php if ($can_see_prices): ?>
                                LKR
                                <?= !empty($pricing_tiers) ? number_format($pricing_tiers[0]['price']) : number_format($product['base_price']) ?>
                            <?php else: ?>
                                <span style="filter: blur(4px); user-select: none; pointer-events: none;"
                                    class="select-none pointer-events-none">LKR
                                    <?= !empty($pricing_tiers) ? number_format($pricing_tiers[0]['price']) : number_format($product['base_price']) ?></span>
                            <?php endif; ?>
                        </span>
                    </div>
                    <div class="flex justify-between text-xs font-bold text-gray-400 uppercase tracking-widest">
                        <span>Total Quantity</span>
                        <span id="order-qty" class="text-gray-900">0 units</span>
                    </div>
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span id="subtotal" class="text-2xl font-extrabold text-brand font-sans">
                            <?php if ($can_see_prices): ?>
                                LKR <span id="subtotal-val">0</span>
                            <?php else: ?>
                                <span style="filter: blur(4px); user-select: none; pointer-events: none;"
                                    class="select-none pointer-events-none">LKR 0</span>
                            <?php endif; ?>
                        </span>
                    </div>
                    <p class="text-[10px] font-medium text-gray-400 leading-tight">VAT and island-wide delivery
                        calculated at final checkout.</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="col-span-2 flex items-start gap-3 p-4 bg-amber-50 border border-amber-200
                            ...
                            <div>
                                ...
                                <p class="text-xs font-medium mt-1 text-amber-700">
                                    Your wholesale account is currently under review by our administrative team. No further
                                    action is required on your part. We typically verify and approve accounts within 24
                                    business hours. If you require urgent access, please <a href="/contact"
                                        class="underline hover:text-amber-900">contact us</a>.
                                </p>
                            </div>
                        </div>
                    ...
                    <?php else: ?>
                        ...
                        <a href="/login" class="bg-brand text-brand-light font-bold py-4 rounded-2xl hover:bg-brand-dark
                ...
                <div class="flex justify-between mt-8">
                    <div
                        class="flex flex-col items-center gap-2 text-[9px] font-bold text-gray-400 uppercase tracking-widest">
...
<div id="quoteModal" class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 items-center justify-center p-4 hidden">
    <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-2xl max-w-lg w-full">
        ...
        <p class="text-xs text-gray-500 mb-6">Enter your details and our team will get back to you with custom pricing.
        </p>
        ...
        <form onsubmit="submitQuoteRequest(event)" class="space-y-4">
            ...
            <div>
                <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Email Address
                    *</label>
            ...
            <div>
                <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Quote Request
                    Message *</label>
                <textarea id="quoteMessage" rows="3" required
                    class="w-full px-4 py-2.5 bg-gray-50 border-none rounded-xl text-sm font-semibold outline-none focus:ring-2 focus:ring-brand/20 resize-none"></textarea>
            </div>
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="submit" id="submitQuoteBtn"
                    class="px-6 py-2.5 bg-brand text-brand-light rounded-xl text-sm font-bold shadow-lg shadow-brand/20">Submit
                    Request</button>
            </div>
        </form>
    </div>
