...
if (isset($pdo) && $pdo !== null) {
    try {
        ...
        $stmt = $pdo->query("SELECT p.name, i.size, i.colour, i.quantity, i.restock_min 
                             FROM inventory i 
                             JOIN products p ON i.product_id = p.id 
                             WHERE i.quantity > i.restock_min * 0.15 AND i.quantity <= i.restock_min 
                             LIMIT 5");
...
<main id="dashboard-container" class="flex-1 overflow-y-auto bg-white">
    ...
    <!-- Header -->
    <header
        class="bg-white border-b border-gray-100 px-4 md:px-10 py-4 md:py-6 sticky top-0 z-30 flex flex-col sm:flex-row sm:items-center justify-between gap-4 md:gap-6">
        <div>
            <h1 class="text-xl font-bold text-gray-900 tracking-tight">System Overview</h1>
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-widest mt-1"><?= date('l, d M Y') ?>
                &nbsp;·&nbsp; Last updated 9:41 AM</p>
        </div>
        <div class="flex items-center gap-4">
            <div class="relative">
                <i class="ti ti-calendar absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-lg"></i>
                <select id="dashboard-filter" onchange="loadDashboardAjax(this.value)"
                    class="bg-gray-50 border border-gray-100 rounded-2xl pl-12 pr-6 py-3 text-xs font-bold text-gray-500 uppercase tracking-widest outline-none appearance-none cursor-pointer disabled:opacity-50">
                    ...
                    <option value="last_quarter" <?= $filter_month === 'last_quarter' ? 'selected' : '' ?>>Last Quarter
                    </option>
                </select>
            </div>
    </header>
    ...
    <div class="p-4 md:p-10 space-y-6 md:space-y-10">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
            <div class="bg-white border border-gray-100 rounded-3xl p-5 md:p-8 shadow-sm">
                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Revenue
                    (<?= htmlspecialchars($month_label) ?>)</p>
                <div class="flex items-baseline gap-3">
                    <h3 class="text-lg 2xl:text-2xl font-bold 2xl:font-extrabold text-gray-900">
                        <?= $revenue_formatted ?></h3>
                    <span
                        class="text-[10px] font-bold text-green-500 bg-green-50 px-2 py-0.5 rounded-full border border-green-100 border-none relative inline-block shrink-0 ring-0 ring-offset-0">+18%</span>
                </div>
                <p class="text-[10px] font-medium text-gray-300 mt-2">vs previous month</p>
            </div>
            <div class="bg-white border border-gray-100 rounded-3xl p-5 md:p-8 shadow-sm">
                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Orders
                    (<?= htmlspecialchars($month_label) ?>)</p>
                <div class="flex items-baseline gap-3">
                    <h3 class="text-lg 2xl:text-2xl font-extrabold text-gray-900">
                        <?= htmlspecialchars($current_month_orders) ?></h3>
                    <span
                        class="text-[10px] font-bold text-green-500 bg-green-50 px-2 py-0.5 rounded-full border border-green-100 border-none relative inline-block shrink-0 ring-0 ring-offset-0">+9</span>
                </div>
                <p class="text-[10px] font-medium text-gray-300 mt-2">vs previous month</p>
            </div>
            <div class="bg-white border border-gray-100 rounded-3xl p-5 md:p-8 shadow-sm">
                ...
                <div class="flex items-baseline gap-3">
                    <h3 class="text-lg 2xl:text-2xl font-extrabold text-gray-900">
                        <?= htmlspecialchars($pending_payment_count) ?></h3>
                    <span
                        class="text-[10px] font-bold text-amber-500 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-100">ACTION</span>
                </div>
            ...
            <div class="bg-white border border-gray-100 rounded-3xl p-5 md:p-8 shadow-sm">
                ...
                <div class="flex items-baseline gap-3">
                    <h3 class="text-lg 2xl:text-2xl font-extrabold text-gray-900">
                        <?= htmlspecialchars($pending_registration_count) ?></h3>
                    <span
                        class="text-[10px] font-bold text-blue-500 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100">REVIEW</span>
                </div>
        ...
        <div class="space-y-4">
            <?php if (!empty($critical_stock_items)): ?>
                <div class="bg-red-50 border border-red-200 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-red-500 flex items-center justify-center shrink-0 shadow-md shadow-red-500/20">
                            <i class="ti ti-alert-triangle text-white text-lg"></i>
                        </div>
                        <p class="text-xs font-bold text-red-900 leading-relaxed">
                            <span class="uppercase tracking-wider font-extrabold text-red-700 mr-1">[Critical Stock Alert]</span>
                            <?= count($critical_stock_items) ?> products critically low on stock:
                            <span class="font-black text-red-950">
                                <?php
                                $crit_names = [];
                                foreach ($critical_stock_items as $item) {
                                    $colour = !empty($item['colour']) ? htmlspecialchars($item['colour']) : '';
                                    $size = !empty($item['size']) ? htmlspecialchars($item['size']) : '';
                                    $variant_str = ($colour && $size) ? "{$colour} / {$size}" : ($colour ?: $size);
                                    $crit_names[] = htmlspecialchars($item['name']) . ($variant_str ? " ({$variant_str})" : "");
                                }
                    ...
                    <a href="/admin-purchase-orders" class="px-4 py-2 bg-red-600 text-white rounded-xl text-xs font-bold hover:bg-red-700 transition-all shrink-0 shadow-sm flex items-center justify-center gap-1.5">
                        <i class="ti ti-plus text-sm"></i> Restock Now
                    </a>
                </div>
            <?php endif; ?>
            ...
            <?php if (!empty($low_stock_items)): ?>
                <div class="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-amber-500 flex items-center justify-center shrink-0 shadow-md shadow-amber-500/20">
                            <i class="ti ti-alert-circle text-white text-lg"></i>
                        </div>
                        <p class="text-xs font-bold text-amber-900 leading-relaxed">
                            <span class="uppercase tracking-wider font-extrabold text-amber-700 mr-1">[Low Stock Warning]</span>
                            <?= count($low_stock_items) ?> products running low on stock:
                            <span class="font-black text-amber-950">
                                ...
                                foreach ($low_stock_items as $item) {
                                    ...
                                    $size = !empty($item['size']) ? htmlspecialchars($item['size']) : '';
                                    $variant_str = ($colour && $size) ? "{$colour} / {$size}" : ($colour ?: $size);
                                    $low_names[] = htmlspecialchars($item['name']) . ($variant_str ? " ({$variant_str})" : "");
                                }
                                echo implode(", ", $low_names);
                                ?>
                            </span>. Consider restocking soon.
                        </p>
                    </div>
                    <a href="/admin-purchase-orders" class="px-4 py-2 bg-amber-600 text-white rounded-xl text-xs font-bold hover:bg-amber-700 transition-all shrink-0 shadow-sm flex items-center justify-center gap-1.5">
                        <i class="ti ti-plus text-sm"></i> Restock
                    </a>
                </div>
            <?php endif; ?>
            ...
            <?php if (!empty($pending_followup_orders)): ?>
                <div class="bg-blue-50 border border-blue-100 rounded-2xl p-4 flex items-center gap-4">
                    ...
                    <p class="text-xs font-bold text-blue-700">
                        Order <span
                            class="font-black underline">KE-2025-<?= str_pad($pending_followup_orders[0]['id'], 5, '0', STR_PAD_LEFT) ?></span>
                        (<?= htmlspecialchars($pending_followup_orders[0]['business_name']) ?>) has been pending. Consider
        ...
        <div class="grid lg:grid-cols-[1fr_400px] gap-6 lg:gap-8">
            ...
            <div class="bg-white border border-gray-100 rounded-3xl p-5 md:p-10 shadow-sm relative overflow-hidden">
                <div class="relative z-10">
                    <div class="flex items-center justify-between mb-10">
                        <h2 class="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">Revenue Performance
                            (LKR)</h2>
                    ...
                    <div class="h-[300px]">
                        <canvas id="revenueChart"
                            data-months='<?= htmlspecialchars(json_encode($chart_months), ENT_QUOTES, 'UTF-8') ?>'
                            data-revenue='<?= htmlspecialchars(json_encode($chart_revenue), ENT_QUOTES, 'UTF-8') ?>'></canvas>
                    </div>
                </div>
            </div>
            ...
            <!-- Status Donut Chart -->
            <div
                class="bg-white border border-gray-100 rounded-3xl p-5 md:p-10 shadow-sm relative overflow-hidden flex flex-col">
                <h2 class="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-10 text-center">Order
                    Status Distribution</h2>
                <div class="flex-1 min-h-[220px]">
                    <canvas id="statusChart" data-pending="<?= (int) $status_counts['Pending'] ?>"
                        data-processing="<?= (int) $status_counts['Processing'] ?>"
                        data-shipped="<?= (int) $status_counts['Shipped'] ?>"
                        data-delivered="<?= (int) $status_counts['Delivered'] ?>"></canvas>
                </div>
                <div class="grid grid-cols-2 gap-4 mt-10">
                    <div class="flex items-center gap-3">
                        <div class="w-2 h-2 rounded-full bg-amber-500"></div>
                        <span class="text-[10px] font-bold text-gray-400 uppercase">Pending
                            (<?= $status_counts['Pending'] ?>)</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-2 h-2 rounded-full bg-blue-500"></div>
                        <span class="text-[10px] font-bold text-gray-400 uppercase">Processing
                            (<?= $status_counts['Processing'] ?>)</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-2 h-2 rounded-full bg-brand"></div>
                        <span class="text-[10px] font-bold text-gray-400 uppercase">Delivered
                            (<?= $status_counts['Delivered'] ?>)</span>
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-2 h-2 rounded-full bg-indigo-500"></div>
                        <span class="text-[10px] font-bold text-gray-400 uppercase">Shipped
                            (<?= $status_counts['Shipped'] ?>)</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid lg:grid-cols-2 gap-6 lg:gap-8">
            ...
            <div class="bg-white border border-gray-100 rounded-3xl p-5 md:p-10 shadow-sm overflow-hidden">
                <div class="flex items-center justify-between mb-8">
                    ...
                    <a href="/admin-orders" class="text-[10px] font-bold text-brand uppercase hover:underline">View
                        All</a>
                </div>
                <div class="divide-y divide-gray-50">
                    <?php foreach ($live_orders as $o): ?>
                        <div class="py-4 flex items-center justify-between group">
                            <div class="flex items-center gap-4">
                                <div
                                    class="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-gray-400 group-hover:bg-brand-light group-hover:text-brand transition-all">
                                    <i class="ti ti-package"></i>
                                </div>
                                <div>
                                    <h4 class="text-sm font-bold text-gray-900 group-hover:text-brand transition-colors">
                                        KE-2025-<?= str_pad($o['id'], 5, '0', STR_PAD_LEFT) ?></h4>
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">
                                        <?= htmlspecialchars($o['business_name']) ?></p>
                                </div>
                            </div>
                            <div class="flex items-center gap-6">
                                ...
                                ?>
                                <span
                                    class="px-3 py-1 text-[10px] font-bold rounded-full border <?= $status_class ?>"><?= strtoupper($o['status']) ?></span>
                                <button class="p-2 text-gray-300 hover:text-brand transition-colors"><i
            ...
            <div class="bg-white border border-gray-100 rounded-3xl p-5 md:p-10 shadow-sm overflow-hidden">
                <div class="flex items-center justify-between mb-8">
                    ...
                    <a href="/admin-inventory"
                        class="text-[10px] font-bold text-brand uppercase hover:underline">Restock All</a>
                </div>
                <div class="space-y-8">
                    <?php foreach ($inventory_pressure as $item): ?>
                        ...
                        <div>
                            <div class="flex justify-between items-center mb-3">
                                <h4 class="text-xs font-bold text-gray-900"><?= htmlspecialchars($item['name']) ?>
                                    (<?= htmlspecialchars($item['size']) ?> · <?= htmlspecialchars($item['colour']) ?>)</h4>
                                <span
                                    class="text-xs font-bold <?= $text_color ?>"><?= htmlspecialchars($item['quantity']) ?>
                                    pcs</span>
                            </div>
                            ...
                            <p class="text-[9px] font-bold text-gray-300 uppercase tracking-widest mt-2">Target:
                                <?= htmlspecialchars($item['restock_min']) ?> Units</p>
                        </div>
                    <?php endforeach; ?>
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
                <?php foreach ($pending_registrations as $reg): ?>
                    <a href="/admin-customers"
                        class="block p-6 bg-gray-50 rounded-3xl border border-gray-100 hover:border-brand/30 transition-all group">
                        <div class="flex justify-between items-start mb-4">
                            <h4 class="text-sm font-bold text-gray-900 group-hover:text-brand transition-colors">
                                <?= htmlspecialchars($reg['business_name']) ?></h4>
                        ...
                        <div class="space-y-1 mb-6">
                            <p class="text-[10px] font-bold text-gray-500 uppercase">
                                <?= htmlspecialchars($reg['business_type']) ?> · <?= htmlspecialchars($reg['br_number']) ?>
                            </p>
                            <p class="text-[11px] text-gray-400 font-medium truncate"><?= htmlspecialchars($reg['email']) ?>
                            </p>
                        </div>
                    </a>
                <?php endforeach; ?>
