...
if (isset($pdo) && $pdo !== null) {
    try {
        ...
        $low_stock_items = [];
        try {
            $stmt_crit = $pdo->query("SELECT p.name, i.size, i.colour, i.quantity, i.restock_min 
                                      FROM inventory i 
                                      JOIN products p ON i.product_id = p.id 
                                      WHERE i.quantity <= i.restock_min * 0.15 
                                      LIMIT 5");
            $critical_stock_items = $stmt_crit->fetchAll();

            $stmt_low = $pdo->query("SELECT p.name, i.size, i.colour, i.quantity, i.restock_min 
                                     FROM inventory i 
                                     JOIN products p ON i.product_id = p.id 
        ...
        foreach ($pos_db as $po) {
            $po_year = !empty($po['ordered_at']) ? date('Y', strtotime($po['ordered_at'])) : date('Y');
...
<div class="flex-1 flex overflow-hidden">
    ...
    <div id="purchase-orders-list-container" class="flex-1 flex flex-col min-w-0 bg-white">
        ...
        <div class="px-8 py-6 border-b border-gray-100 flex items-center justify-between">
            ...
            <div class="flex items-center gap-6">
                ...
                <div class="flex items-center gap-3 border-l border-gray-100 pl-6">
                    <button onclick="downloadPDF('purchase-orders-list-container', 'Purchase_Orders_List')"
                    ...
                    <button onclick="openRaisePOModal()"
        ...
        <?php if (!empty($critical_stock_items)): ?>
            <div
                class="mx-8 mt-6 p-4 bg-red-50 border border-red-200 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-xl bg-red-500 flex items-center justify-center shrink-0 shadow-md shadow-red-500/20">
                ...
                <button onclick="openRaisePOModal()"
                    class="px-4 py-2 bg-red-600 text-white rounded-xl text-xs font-bold hover:bg-red-700 transition-all shrink-0 shadow-sm flex items-center justify-center gap-1.5">
        ...
        <?php if (!empty($low_stock_items)): ?>
            <div
                class="mx-8 mt-6 p-4 bg-amber-50 border border-amber-200 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-xl bg-amber-500 flex items-center justify-center shrink-0 shadow-md shadow-amber-500/20">
                ...
                <button onclick="openRaisePOModal()"
                    class="px-4 py-2 bg-amber-600 text-white rounded-xl text-xs font-bold hover:bg-amber-700 transition-all shrink-0 shadow-sm flex items-center justify-center gap-1.5">
        ...
        <div class="px-8 py-4 border-b border-gray-100 flex items-center gap-2 overflow-x-auto bg-gray-50/30">
            <button onclick="chipFilter(this)"
                class="chip px-4 py-2 bg-brand text-brand-light rounded-xl text-xs font-bold shadow-md shadow-brand/10 on transition-all border border-transparent">All</button>
            <button onclick="chipFilter(this)"
                class="chip px-4 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-500 hover:bg-gray-50 transition-all">Sent</button>
            <button onclick="chipFilter(this)"
                class="chip px-4 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-500 hover:bg-gray-50 transition-all">Partial</button>
            <button onclick="chipFilter(this)"
                class="chip px-4 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-500 hover:bg-gray-50 transition-all">Received</button>
            <button onclick="chipFilter(this)"
                class="chip px-4 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-500 hover:bg-gray-50 transition-all">Overdue</button>
        </div>

        <!-- List Pane Body -->
        <div class="flex-1 overflow-y-auto overflow-x-auto no-scrollbar pb-10" id="purchase-orders-table-scroll">
                ...
                <table class="w-full text-left border-separate" style="border-spacing: 0 4px;">
                    ...
                    <tbody id="po-list">
                        <?php if (empty($admin_pos)): ?>
                            <tr id="empty-state">
                                <td colspan="5" class="p-12 text-center text-gray-400 text-sm">No purchase orders found.
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($admin_pos as $po): ?>
                                <tr class="po-row bg-white cursor-pointer hover:bg-gray-50/50 transition-all group shadow-sm"
                                    id="po-row-<?= $po['id'] ?>" onclick="selectPO(this)" data-id="<?= $po['id'] ?>"
                                    ...
                                    <td class="p-4 border-y border-gray-100 group-hover:border-brand/30">
                                        <p
                                            class="text-xs font-semibold <?= $po['expectedColor'] ?> bg-gray-50 px-2 py-1 rounded-lg border w-max">
                                            <?= $po['expected'] ?></p>
                                    </td>
                                    ...
                                    <td
                                        ...
                                        <span
                                            class="px-3 py-1 rounded-full text-[9px] font-bold border <?= $po['badge'] ?> uppercase tracking-wider whitespace-nowrap shadow-sm"><?= $po['badgeText'] ?></span>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
    ...
    <!-- PO Details Backdrop -->
    <div id="po-detail-backdrop"
        class="hidden fixed inset-0 bg-black/40 z-40 backdrop-blur-[2px] transition-opacity duration-300"
        onclick="closePODetailPane()"></div>

    <!-- PO Details Pane (Right) -->
    <div id="po-detail-pane"
        class="fixed inset-y-0 right-0 z-50 w-1/2 max-w-full bg-white flex flex-col shadow-2xl transform translate-x-full transition-transform duration-300 overflow-y-auto border-l border-gray-200">
        <div id="detail-print-area" class="p-8 flex-1 overflow-y-auto space-y-8 relative">
            <button onclick="closePODetailPane()"
            ...
            <!-- Warning Banner -->
            <div id="d-alert"
                class="p-4 rounded-2xl flex items-start gap-3 border text-xs bg-blue-50 border-blue-100 text-blue-700">
        ...
        <div class="p-6 border-t border-gray-100 bg-gray-50/50 space-y-3 print:hidden">
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <button onclick="resendPO()"
                    class="flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-700 hover:bg-gray-50 transition-all">
                    <i class="ti ti-mail text-base"></i>
                    Resend
                </button>
            </div>
            <button id="d-cancel-btn" onclick="cancelPO()"
                class="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-red-100 rounded-xl text-xs font-bold text-red-600 hover:bg-red-50 transition-all">
    ...
    <div class="bg-white p-8 rounded-3xl border border-gray-100 shadow-2xl max-w-2xl w-full">
        ...
        <form method="POST" data-turbo="false">
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                <button type="button" onclick="closeRaisePOModal()"
                    class="px-6 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-50 transition-all">Cancel</button>
                <button type="submit"
                    class="px-6 py-2.5 bg-brand text-brand-light rounded-xl text-sm font-bold shadow-lg shadow-brand/20 hover:opacity-90 transition-all">Raise
                    Order</button>
            </div>
        </form>
    </div>
